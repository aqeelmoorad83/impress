  ...
  loadApplications(() => {
    ...
    if (impress.serverName && impress.serverName.indexOf('test') !== -1) {
      ...
      api.test.test('Impress applications test', test => {
        ...
        for (const appName in impress.applications) {
          if (!impress.applications.hasOwnProperty(appName)) {
            continue;
          }

          const app = impress.applications[appName];
          test.test(appName, test => {
            test.endAfterSubtests();

            for (const { fileName: name, test: unittest } of app.tests.unit) {
              test.test(name, unittest);
            }
          });
        }
      });
