...
class Application extends api.events.EventEmitter {
    ...
    impress.createSandbox(this, () => {
      ...
      this.config.loadConfig(() => {
        ...
        api.metasync.sequential([
          ...
          callback => this.loadLib(null, callback),
  ...
  loadLib(path, callback) {
    if (!path) {
      path = api.path.join(this.dir, 'lib');
    }
    api.fs.readdir(path, (err, files) => {
      ...
      api.metasync.each(files, (fileName, cb) => {
        ...
        api.fs.stat(filePath, (err, stats) => {
          ...
          this.loadLibFile(fileName, filePath, cb);
        });
      }, callback);
      // ? this.cache.watch(path);
    });
  }

  loadLibFile(fileName, path, callback) {
    ...
    if (isTest) {
      ...
      impress.createScript(this, path, (err, test) => {
        ...
        this.tests.unit.push({ fileName, test });
        callback(null);
      });
    } else {
      impress.createScript(this, path, err  => {
        if (err) {
          callback(err);
          return;
        }
